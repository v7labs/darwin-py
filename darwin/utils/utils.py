...
def get_image_path_from_stream(
    ...
    try:
        if json_version == "2.0":
            # filename = Path(darwin_json["item"]["name"])
            filename = Path(darwin_json["item"]["slots"][0]["source_files"][0]["file_name"])
            if not with_folders:
...
def split_video_annotation(annotation: dt.AnnotationFile) -> List[dt.AnnotationFile]:
    ...
    for i, frame_url in enumerate(urls):
        ...
        frame_slots = annotation.slots
        frame_slots[0].source_files[0]["file_name"] = filename

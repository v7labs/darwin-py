...
class TestUploadFiles(SetupTests):
    def test_async_upload_files(self, base_client: ClientCore, tmp_path: Path) -> None:
        ...
        with aioresponses() as mocked:
            ...
            response: ClientResponse = asyncio.run(
                uploads.async_upload_file(base_client, "url", file)
            )
    ...
    def test_async_upload_files_raises(
        self, base_client: ClientCore, tmp_path: Path
    ) -> None:
        with patch("darwin.future.core.items.uploads.aiohttp.ClientSession") as mock:
            ...
            with pytest.raises(DarwinException):
                asyncio.run(
                    uploads.async_upload_file(
                        base_client, "url", tmp_path / "file1.txt"
                    )
                )

...
def test_get_item(base_items_json: List[dict], base_items: List[Item], base_client: ClientCore) -> None:
...
def test_list_items(base_items_json: List[dict], base_items: List[Item], base_client: ClientCore) -> None:
    with responses.RequestsMock() as rsps:
        rsps.add(
            rsps.GET,
            base_client.config.api_endpoint + f"v2/teams/default-team/items?dataset_ids=1337",
            json={"items": base_items_json},
            status=200,
        )
        items = list_items(base_client, "default-team", QueryString({"dataset_ids": "1337"}))
        for item, comparator in zip(items, base_items):
            assert item == comparator


def test_list_folders(base_folders_json: List[dict], base_folders: List[Folder], base_client: ClientCore) -> None:
    with responses.RequestsMock() as rsps:
        rsps.add(
            rsps.GET,
            base_client.config.api_endpoint + f"v2/teams/default-team/items/folders?dataset_ids=1337",
            json={"folders": base_folders_json},
            status=200,
        )
        folders = list_folders(base_client, "default-team", QueryString({"dataset_ids": "1337"}))

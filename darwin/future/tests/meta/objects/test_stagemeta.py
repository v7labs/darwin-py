...
from darwin.future.data_objects.workflow import WFStageModel, WFType
...
@fixture
def base_WFStage(uuid_str: str) -> WFStageModel:
    return WFStageModel(id=UUID(uuid_str), name="test-stage", type=WFType.ANNOTATE, assignable_users=[], edges=[])


@fixture
def stage_meta(base_meta_client: Client, base_WFStage: WFStageModel, workflow_id: UUID) -> StageMeta:
    return StageMeta(

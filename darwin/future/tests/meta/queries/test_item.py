...
def item_core_list() -> List[ItemCore]:
    ...
    for i in range(5):
        ...
        item = ItemCore(name=f'item_{i}', id=uuid4(), slots=[slot], dataset_id=i, processing_status='processed', layout=layout)
        items.append(item)
    return items

@pytest.fixture
def items(base_client: ClientCore, item_list: List[ItemCore]) -> List[Item]:
    return [Item(client=base_client, element=item, meta_params={'team_slug': 'test', 'dataset_id': 1}) for item in item_list]
...
def items_json(item_core_list: List[ItemCore]) -> List[dict]:
    ...
    for item in item_core_list:
        item = item.dict()
        item['id'] = str(item['id'])
        item['slots'] = [dict(slot) for slot in item['slots']]
        item['layout'] = dict(item['layout'])
        items.append(item)
    return items
@pytest.fixture
def item_query(base_client: ClientCore) -> ItemQuery:
    return ItemQuery(client=base_client, meta_params={'team_slug': 'test', 'dataset_id': 1})
...
def test_item_query_collect(item_query: ItemQuery, items_json: List[dict]) -> None:
    with responses.RequestsMock() as rsps:
        rsps.add(
            ...
            match=[query_param_matcher({'page[offset]': '0', 'page[size]': '500', 'dataset_ids': '1'})],

...
def test_integrated_parsing_works_with_raw(basic_combined: dict) -> None:
    team = TeamModel.parse_obj(basic_combined)
...
def test_broken_obj_raises(broken_combined: dict) -> None:
    with pytest.raises(ValidationError) as e_info:
        broken = TeamModel.parse_obj(broken_combined)


@pytest.mark.parametrize("test_object", [TeamModel, DatasetModel, ReleaseModel])

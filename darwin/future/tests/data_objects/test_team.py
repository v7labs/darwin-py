...
def test_get_team_returns_valid_team(base_client: CoreClient, base_team_json: dict, base_team: TeamCore) -> None:
    slug = "test-slug"
...
def test_get_team_fails_on_incorrect_input(base_client: CoreClient, base_team: TeamCore) -> None:
    slug = "test-slug"
    endpoint = base_client.config.api_endpoint + f"teams/{slug}"
    with responses.RequestsMock() as rsps:
...
def test_team_from_client(base_client: CoreClient, base_team_json: dict, base_team: TeamCore) -> None:
    with responses.RequestsMock() as rsps:
        rsps.add(
            responses.GET,

...
class LocalFile:
    ...
    def _type_check(self, args):
        ...
        self.data["type"] =  args.get("type") or self._infer_type()  

    def _infer_type(self):
        extension = Path(self.data["filename"]).suffix
        if is_image_extension_allowed(extension):
            return "image"
        if is_video_extension_allowed(extension):
            return "video"
        raise UnsupportedFileType(self.data["filename"])
...
class UploadHandler:
    def __init__(self, client, local_files, dataset_identifier):
        ...
        print(self.blocked_items, self.pending_items)
    ...
    def upload(self):
        return _upload_files(self.client, self.local_files, self.pending_items, self.dataset_identifier.team_slug)
    

def request_upload(client: "Client", files: List[LocalFile], dataset_identifier):
    upload_payload = { "items": [file.data for file in files] }
    data = client.put(endpoint=f"/teams/{dataset_identifier.team_slug}/datasets/{dataset_identifier.dataset_slug}/data", payload=upload_payload, team=dataset_identifier.team_slug)
    return data["blocked_items"], data["items"]

# def add_files_to_dataset(client: "Client", files: List[LocalFile], dataset_slug: str, team_slug: str):
#     upload_payload = { "items": [file.data for file in files] }
#     data = client.put(endpoint=f"/teams/{team_slug}/datasets/{dataset_slug}/data", payload=upload_payload, team=team_slug)
    
#     print(data)

#     if 'items' in data and data['items']:
#         _upload_files(client, files, data['items'], team_slug)

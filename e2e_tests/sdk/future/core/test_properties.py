...
def base_config(tmpdir: str) -> DarwinConfig:
    ...
    return DarwinConfig(api_key=api_key, base_url=server, api_endpoint=server + "/api/", datasets_dir=Path(tmpdir), default_team=team_slug, teams={})
...
def base_property_to_create() -> FullProperty:
    return FullProperty(
        ...
        options=None,
        property_values=[PropertyValue(id=None, position=None, type="string", value="test_value", color="rgba(100,100,100,1)")],
    )
...
def test_create_property(base_client: ClientCore, base_property_to_create: FullProperty) -> None:
    ...
    output = create_property(base_client, base_property_to_create, team_slug=config.team_slug)
    assert isinstance(output, FullProperty)
    
    
def test_get_team_properties(base_client: ClientCore, base_property_to_create: FullProperty) -> None:
    ...
    base_property_to_create.annotation_class_id = new_annotation_class.id
    output = create_property(base_client, base_property_to_create, team_slug=config.team_slug)
    output.property_values = None # the base get_team_properties doesn't include property_values
    assert isinstance(output, FullProperty)
...
def test_get_team_full_properties(base_client: ClientCore, base_property_to_create: FullProperty) -> None:
    config, new_annotation_class = helper_create_annotation("test_for_get_full_properties")
...
def test_get_property_by_id(base_client: ClientCore, base_property_to_create: FullProperty) -> None:
    config, new_annotation_class = helper_create_annotation("test_for_get_property_by_id")
...
def test_update_property(base_client: ClientCore, base_property_to_create: FullProperty) -> None:
...
def test_update_property_value(base_client: ClientCore, base_property_to_create: FullProperty) -> None:
    config, new_annotation_class = helper_create_annotation("test_for_update_property_value")
    ...
    pv.value = "new_value"
    new_output = update_property_value(base_client, pv, item_id=id, team_slug=config.team_slug)
